...
namespace Pcx
{
    ...
    public sealed class PointCloudData4D : ScriptableObject
    {
        ...
        public const int elementSize = sizeof(float) * 4 + sizeof(uint);
        ...
        [SerializeField] Point4D[] _pointData;
        ...
        public void Initialize(List<Vector4> positions, List<Color32> colors)
        {
            _pointData = new Point4D[positions.Count];
            for (var i = 0; i < _pointData.Length; i++)
            {
                _pointData[i] = new Point4D
                {
